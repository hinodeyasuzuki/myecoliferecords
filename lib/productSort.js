...
const COMPARATORS = {
  ...
  repaired: (itemA, itemB) =>
    compareValues(itemA.repairlog_ids.length > 0, itemB.repairlog_ids.length > 0),
...
export function sortProductEntries(entries, sortKey, sortDir, equipsById, methodOptions) {
  const copy = [...entries];
  ...
  copy.sort(([, itemA], [, itemB]) => {
    ...
    return dirMultiplier * comparator(itemA, itemB, equipsById, methodOptions);
  });
  return copy;
}
